...
public:
    ...
    int health;        // 적의 체력
    int damage;        // 적의 피해량
    int directionX, directionY;  // 적의 이동 방향
    bool active;       // 적이 활성화 상태인지 여부
    int shootInterval; // 적의 총알 발사 간격
    int shootTimer;    // 총알 발사 타이머
    ...
    virtual void draw() = 0;  // 가상 함수로 적의 구체적인 구현은 하위 클래스에서
    ...
    void takeDamage(int dmg) 
    {
        ...
        if (health <= 0) 
        {
            active = false;  // 체력이 0 이하로 떨어지면 비활성화
        }
    }
...
public:
    ...
    void draw() override 
    {
        if (active) 
        {
            ...
            std::cout << "|=(_)=|" << std::endl;
            gotoxy(x, y+2);
            std::cout << "|     |" << std::endl;  // EnemyA의 시각적 표현
            TextColor3(15, 0);
